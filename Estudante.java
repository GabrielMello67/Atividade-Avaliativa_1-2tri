...
public class Estudante {
    ...
    public double obterMedia(int[] valoresPeso){

        double somaNotas = 0;
        double[] listaNotas = this.getNotas();
        int totalPesos=0;
        for(int i=0;i<quantidadeNotas;i++){

            somaNotas+=listaNotas[i]*valoresPeso[i];
            totalPesos+=valoresPeso[i];

        }

        somaNotas/=totalPesos;
